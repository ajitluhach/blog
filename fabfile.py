...
def publish():
    ...
    local('echo luhach.me > {deploy_path}/CNAME'.format(**env))
    local("echo $'User-agent: *\nSitemap: http://luhach.me' > {deploy_path}/robots.txt".format(**env))
    local("ghp-import {deploy_path}".format(**env))
    local("git push git@github.com:ajitluhach/ajitluhach.github.io.git gh-pages:master -f")
